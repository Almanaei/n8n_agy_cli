...
async function run() {
  ...
  const getRes = await fetch(`https://sheets.googleapis.com/v4/spreadsheets/${spreadsheetId}/values/Sheet1!A1:I1000`, {
    headers: { "Authorization": `Bearer ${accessToken}` }
  });
  
  if (!getRes.ok) {
    console.error("Get values failed:", await getRes.text());
    return;
  }
  ...
  console.log("\nRows with empty name or phone (excluding header):");
  for (let i = 1; i < rows.length; i++) {
    ...
    const convId = row[4];
    if (!name || !phone) {
      console.log(`Row ${i + 1}: Name="${name || ''}", Phone="${phone || ''}", ConvID="${convId || ''}", Email="${row[3] || ''}"`);
    }
  }
}
run();
